...
let imagePaths = []; // store all image paths
...
uploadImages.forEach((fileupload, index) => {
    fileupload.addEventListener('change', () => {
        ...
        if(file.type.includes('image')){ // if users uploads an image
...
const validateForm = () => {
    if(!productName.value.length){
        ...
        return showAlert('upload at least one image of the product');
        ...
        return showAlert('you must add a price for the product');
    } else if(stock.value < 1){
        showAlert('you should have at least 1 item in stock');
    } else if(!tags.value.length){
        return showAlert('enter tags to help rank your product in search list');
    } else if(!tac.checked){
        return showAlert('you must accept terms and conditions to proceed');
    }
    return true;
}
...
addProductBtn.addEventListener('click', () => {
    storeSizes();
    // validate form
    if(validateForm()){// validate form returns true or false
