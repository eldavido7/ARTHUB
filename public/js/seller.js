...
const address = document.querySelector('#artist-add');
...
applyFormButton.addEventListener('click', () => {
    if(!artistName.value.length || !address.value.length || 
        !about.value.length || !number.value.length){
            showAlert('fill up all fields before proceeding');
        } else if(!tac.checked || !legitInfo.checked){
            showAlert('you must accept terms and conditions to proceed');
